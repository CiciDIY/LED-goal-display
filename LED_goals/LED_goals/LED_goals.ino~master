...
void LED_light_up() {
  ...
  for (int colour_step, LED_target, LED_left = 0; LED_left < NUM_LEDS ; LED_left++) {
    if (current_input >= (LED_target* LED_left)) {
      pixels.setPixelColor(LED_left, pixels.Color(0,255,0));
    }
...
void rainbow() {
    ...
    if (current_input >= goal_input) {
      ...
      pixels.setPixelColor((NUM_LEDS-1), pixels.Color(0,255,255));
      pixels.show();
    }
    if (current_input < goal_input) {
      ...
      pixels.setPixelColor((NUM_LEDS-1), pixels.Color(255,255,0));
      pixels.show();
    }
  }
